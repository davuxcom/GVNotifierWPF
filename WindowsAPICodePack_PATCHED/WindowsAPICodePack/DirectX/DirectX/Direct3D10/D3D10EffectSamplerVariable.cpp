...
SamplerDescription EffectSamplerVariable::GetBackingStore(UInt32 index)
{
    ...
    CommonUtils::VerifyResult(GetInterface<ID3D10EffectSamplerVariable>()->GetBackingStore(
        index, 
        &desc));
...
SamplerState^ EffectSamplerVariable::GetSampler(UInt32 index)
{
    ...
    CommonUtils::VerifyResult(GetInterface<ID3D10EffectSamplerVariable>()->GetSampler(static_cast<UINT>(index), &tempoutSampler));
