...
BlendDescription EffectBlendVariable::GetBackingStore(UInt32 index)
{
    ...
    CommonUtils::VerifyResult(GetInterface<ID3D10EffectBlendVariable>()->GetBackingStore(
...
BlendState^ EffectBlendVariable::GetBlendState(UInt32 index)
{
    ...
    CommonUtils::VerifyResult(
        GetInterface<ID3D10EffectBlendVariable>()->GetBlendState(static_cast<UINT>(index), &tempoutBlendState));
