...
RasterizerDescription EffectRasterizerVariable::GetBackingStore(UInt32 index)
{
    ...
    CommonUtils::VerifyResult(GetInterface<ID3D10EffectRasterizerVariable>()->GetBackingStore(
        index, &desc));
...
RasterizerState^ EffectRasterizerVariable::GetRasterizerState(UInt32 index)
{
    ...
    CommonUtils::VerifyResult(GetInterface<ID3D10EffectRasterizerVariable>()->GetRasterizerState(static_cast<UINT>(index), &tempoutRasterizerState));
