...
public:
    ...
    /// </summary>
    /// <param name="dstRect">In the current bitmap, the upper-left corner of the area to which the region specified is copied.</param>
...
public:
    ...
    /// <summary>
    /// retruns the gradient stops as a readonly collection.
    /// <para>(Also see DirectX SDK: ID2D1GradientStopCollection::GetGradientStops)</para>
    /// </summary>
    /// <returns>A ReadOnlyCollection of <see cref="GradientStop"/>.</returns>
...
public:
    ...
    /// <summary>
    /// Retrieves the bitmap associated with this brush.
...
public:
    ...
    /// </summary>
    /// <param name="size">The dimension of the bitmap to create in pixels.</param>
    /// <param name="sourceBytes">An array of bytes holding the image data.</param>
    /// <param name="pitch">The byte count of each scanline, which is equal to (the image width in pixels * the number of bytes per pixel) + memory padding. (Note that pitch is also sometimes called stride.)</param>
    /// <param name="bitmapProperties">The pixel format and dots per inch (DPI) of the bitmap to create.</param>
    /// <returns>D2DBitmap</returns>
    D2DBitmap ^
    ...
    /// with the render target for the call to succeed. For example, a WICBitmapLock can be
    ...
    /// The render target must be GDI-compatible (the RenderTargetUsage.GdiCompatible flag was specified when creating the render target), 
    /// use the DXGI's Format.B8G8R8A8_UNORM pixel format, and use the AlphaMode.Premultiplied or AlphaMode.Ignore alpha mode.
    ...
    /// </remarks>
    GdiInteropRenderTarget^ GetGdiInteropRenderTarget();
    ...
    bool TryFlush([System::Runtime::InteropServices::Out] D2D::Tags % tags, [System::Runtime::InteropServices::Out] ErrorCode % errorCode);
    ...
    bool
    TryEndDraw([System::Runtime::InteropServices::Out] D2D::Tags % tags, [System::Runtime::InteropServices::Out] ErrorCode % errorCode
        );
    ...
    void DrawGlyphRun(
        ...
        array<DWrite::GlyphRun>^ glyphRun,
...
public:
    ...
    /// </summary>
    /// <returns>The contained <see cref="D2DBitmap"/>.</returns>
    D2DBitmap ^
    GetBitmap(
        );
...
public:
    ...
    /// </summary>
    property cli::array<FLOAT>^ Dashes
    {
        cli::array<FLOAT>^ get();
    }
...
public:
    ...
    /// <param name="flatteningTolerance">The maximum bounds on the distance between points in the polygonal approximation of the geometry. Smaller values produce more accurate results but cause slower execution.</param>
    /// <param name="unitTangentVector">When this method returns, contains a pointer to the tangent vector at the specified distance along the geometry. If the geometry is empty, this vector contains NaN as its x and y values.</param>
...
public:
    ...
    /// </summary>
    /// <returns>The source <see cref="Geometry"/> object.</returns>
    Geometry ^
    GetSourceGeometry(
        );
